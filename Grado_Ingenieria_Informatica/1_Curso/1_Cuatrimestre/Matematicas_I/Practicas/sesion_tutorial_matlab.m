...
V  = round(10*rand(1,10)) % También valdría int8()
VV = int8(10*rand(1,10))

% b) Genera un vector binario donde el 1 binario señala a los elementos
% del vector que son menores o iguales a 5.

VBinario = ones(size(V));  
for i = 1:10  % También vale i = V
    if V(i) > 5
        VBinario(i) = 0;
    %else VBinario(i) = 1;
    end
...
VOrden = V; posicionesV = [];
for i = 1:9
    max = i;
    aux = VOrden(i);
    for j = i+1:10
        if VOrden(j) > VOrden(max)
            max = j;
        end
    end
...
MMedia = mean(mean(M)) % mean(M) crea un vector fila con la media de las columnas!!
...
for i = 1:100
    conteoValores(A4(i)+1) = conteoValores(A4(i)+1) + 1;
end
