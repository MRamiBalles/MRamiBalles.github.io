...
ancho = 10;
largo = 16;
MAPA = imread('.\cuadro_nuevoNuevo.bmp');
...
[x(:,k+1),laser]=kuta(t(k+1),x,h,0,0,handles_plots,handles_coche,handle_vector,[45 0],MAPA);
 ...
 rho=2*delta/LH^2;
  ...
  %------------------------------------
  coordinador(k) = coseno^5; %% 0 para seguimiento reactivo, ^10 para separarse mas de las paredes
  
  volante(k)=(coordinador(k)*volante_tracking(k))+((1-coordinador(k))*volante_reactivo(k));
  
  velocidad(k)=(coordinador(k)*velocidad_traking(k))+10*((1-coordinador(k))*velocidad_reactiva(k));
