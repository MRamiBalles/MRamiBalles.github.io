...
for i = 1:numel(indInteres)
    Io(Ie == indInteres(i)) = indInteres(i); 
end
...
Icolor = zeros(size(Ib)); 
Ibc = Icolor(Ietiq == indCuad);
Ibt = Icolor(Ietiq == indTri);

% Encontrar el centro del cuadrado
centro_cuadrado = round(centroides(indCuad, :));
Ibcent = Icolor(centro_cuadrado(2), centro_cuadrado(1), :); 

Icolor = funcion_visualiza(Io, Ibc, CC, false);
funcion_visualiza(Icolor, Ibt, CT);